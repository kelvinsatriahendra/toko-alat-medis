...
if(isset($_POST['submit1'])){
	...
	$edit = mysqli_query($conn, "UPDATE cart SET qty = '$qty' where id_cart = '$id_cart'");
	if($edit){
			echo "
		<script>
	...
	$id_cart = $_GET['id'];
	$del = mysqli_query($conn, "DELETE FROM cart WHERE id_cart = '$id_cart'");
	if($del){
		echo "
		<script>
	...
	<h2 style=" width: 100%; border-bottom: 4px solid #ff8680"><b>Keranjang</b></h2>
		<table class="table table-striped">
			...
			// CEK JUMLAH KERANJANG
				$cek = mysqli_query($conn, "SELECT * FROM cart WHERE kode_customer = '$kode_cs'");
				...
				if($jml > 0){
					...
					if(isset($_SESSION['kd_cs'])){
						...
						$result = mysqli_query($conn, "SELECT k.id_cart as cart, k.kode_produk as kd, k.nama_produk as nama, k.qty as jml, p.image as gambar, p.harga as hrg FROM cart k join produk p on k.kode_produk=p.kode_produk WHERE kode_customer = '$kode_cs'");
						$no = 1;
						$hasil = 0;
					...
					<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
						...
						<tr>
							...
							<td><button type="submit" name="submit1" class="btn btn-warning">Update</button> | <a href="keranjang.php?del=1&id=<?= $row['cart']; ?>" class="btn btn-danger" onclick="return confirm('Yakin ingin dihapus ?')">Delete</a></td>
						</tr>
						</form>
					<?php 
							$sub = $row['hrg'] * $row['jml'];
